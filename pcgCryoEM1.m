    function [x, error, iter,OutPut] = pcgCryoEM1(A,Minvmat, b, tol, max_it, x0, xtrue,g)
% Fastest version.
%
% Input:
...
r = b- A(x0); %SystemRegMatrixtimesx(A,lambda, x0);
error=norm(r(:));
...
for iter = 1:max_it                       % begin iteration
    ...
    %end
    %% relative error
    re(iter)   = norm(x(:)-xtrue(:))/norm(x(:));
    [MPSNRALLp(iter), SSIMALLp(iter), FSIMALLp(iter)] = quality(real(x)*255, double(xtrue)*255);
%      fprintf( 'Estimated Image: iter = %2.3f, PSNR = %2.2f \n\n\n', iter, MPSNRALLp(iter) );
%      Ax = A*x; 
%      fun_val(iter) = alpha*norm(Ax(:)-g(:),'fro')^2;
end
OutPut.Sol      = x;
OutPut.re        = re;
