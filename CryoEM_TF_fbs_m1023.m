function OutPut = CryoEM_TF_fbs_m1023(A,g,T,b,eta,eta0,sigma,voltrue)
...
%     u = u - p./(2*eta);
    ...
    volnew = volnew-1e-8*(T(volnew) - 2*b);
...
%    p = p + t*(volnew - u);
    ...
    vol  = volnew;
    %t  = t * 1.05; 
    [MPSNRALLp(k), SSIMALLp(k), FSIMALLp(k)] = quality(real(volnew), double(voltrue));
