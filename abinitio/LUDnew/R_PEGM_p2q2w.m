function est_rots = R_PEGM_p2q2w(W,C,n_theta, ref_rot)
% 2019-10-14
%% Object:
%  min  \sum_{i,j} w_{ij}|| RiCij - RjCji||_2^2
%  = min - \sum_i  w_{ij} <Ri, \sum_j (RjCjiCij'>
%  s.t  Ri'Ri = I
% L(R) = - \sum_i w_{ij} <Ri, \sum_j (RjCjiCij'>
% Solution: Projected Extral-gradient algorithm  (PEGM)
%%
...
for t = 0.1%:0.05: 1
    ...
    for iter = 1:100
        for i =1:K
            Grad_Ri = zeros(3,3);
            for j = 1:K
                    Grad_Ri = Grad_Ri +W(i,j)* R_old(:,:,j) *C(:,j,i)*C(:,i,j)';
            end
        ...
        for i =1:K
            Grad_Ri = zeros(3,3);
            for j = 1:K
                    Grad_Ri = Grad_Ri + W(i,j)* R_hat(:,:,j) *C(:,j,i)*C(:,i,j)';
            end
            tmp = R_old(:,:,i) + t * Grad_Ri;
            [U,~,V] = svd(tmp,0);
            R_new(:,:,i) = U*V';
